...
#include "src/envoy/tcp/dubbotest/metadata_exchange.h"
...
#include "src/envoy/tcp/dubbotest/metadata_exchange_initial_header.h"
...
const Http::LowerCaseString APPLICAJSON{"application/json-rpc"};
...
Buffer::Instance &HttpEncoder::response(Extensions::NetworkFilters::DubboProxy::MessageMetadataSharedPtr metadata) {
    response_line();
    message(metadata);
    return http_;
}
...
void HttpEncoder::request_line(const std::string &method, const std::string &uri) {
    http_.drain(http_.length());
    for (const auto &s : {method, SP, uri, SP, HTTP_VERSION, CRLF})
        http_.add(s);
}

void HttpEncoder::response_line() {
    http_.drain(http_.length());
    std::string rl = "200 OK";
    //is_status_line(s) ? s : (s.empty() ? "200 OK" : "200 " + s);
    for (const auto &si : {HTTP_VERSION, SP, rl, CRLF}) {
        http_.add(si);
    }
}

void HttpEncoder::header(const Http::LowerCaseString &name, const std::string &value) {
...
void HttpEncoder::message(Extensions::NetworkFilters::DubboProxy::MessageMetadataSharedPtr metadata) {
    // Do content-related headers first
    content_type_ = APPLICAJSON.get();
    if (!metadata->hasResponseStatus()) {
        content_.clear();
    } else { // Not empty

        content_ = R"("jsonrpc":"2.0","id":"1","result":"123")";
...
AmqpBuilder::AmqpBuilder(enum http_parser_type type) {
...
// @throw EnvoyException
bool AmqpBuilder::parse(Buffer::Instance &http, bool end_stream) {
    done_ = false;
    //Buffer::RawSlice slice;
    //Buffer::RawSlice& slice;
    Buffer::RawSliceVector slices = http.getRawSlices();
    const int num_slices = slices.size();

            ENVOY_LOG(trace, "AmqpBuilder  parse  num_slices {}",
                      num_slices);

    // Decodes header block using nghttp2.
    for (int i = 0; i < num_slices; i++) {
        auto slice = slices[i];
        ENVOY_LOG(trace, "AmqpBuilder  parse  slice mem {}  slice len {} i {}",
                  slice.mem_,slice.len_,i);
        size_t len =
...
int AmqpBuilder::code() const { return parser_.status_code; }

const std::string &AmqpBuilder::status() const { return status_; }


// Reset to parse a new message
void AmqpBuilder::onMessageBegin() {
    // Clear out for new message
    url_.clear();
    metadata_ = std::make_shared<Extensions::NetworkFilters::DubboProxy::MessageMetadata>();
    field_.clear();
    value_.clear();
    status_.clear();
    ...
    //protocol_ = std::make_unique<Extensions::NetworkFilters::DubboProxy::Protocol>();
}

void AmqpBuilder::onUrl(const char *data, size_t length) { url_.append(data, length); }

void AmqpBuilder::onBody(const char *data, size_t length) {
    const uint8_t *begin = reinterpret_cast<const uint8_t *>(data);
    //这里对body进行解析，设置到metadata里
    ENVOY_LOG(trace, "AmqpBuilder  onBody {}, Got {}, begin",
              length,
              data, begin);
    body_.insert(body_.end(), begin, begin + length);
};

void AmqpBuilder::onStatus(const char *data, size_t length) {
    status_.append(data, length);
};

void AmqpBuilder::onHeadersComplete() {
    if (in_value_) {
...
void AmqpBuilder::onHeaderField(const char *data, size_t length) {
    onHeadersComplete(); // Complete the previous header if there was one
    field_.append(data, length);
}

void AmqpBuilder::onHeaderValue(const char *data, size_t length) {
    in_value_ = true;
    value_.append(data, length);
}

void AmqpBuilder::onMessageComplete() {
    done_ = true;

    ENVOY_LOG(trace, "AmqpBuilder  onMessageComplete  body {}",
              body_);

    if (has_content_) {
        //message_.inferred(true); // Data section
        //message_.body(std::move(body_));
        //将body反序列化，然后设置到metadata里
    }
}


Extensions::NetworkFilters::DubboProxy::MessageMetadataSharedPtr AmqpRequestBuilder::message() {

    auto invo = std::make_shared<Extensions::NetworkFilters::DubboProxy::RpcInvocationImpl>();
    auto copy = std::make_shared<Extensions::NetworkFilters::DubboProxy::MessageMetadata>();
    //invo->setServiceName(url_);
    invo->setServiceName("org.apache.dubbo.samples.http.api.DemoService");
    invo->setServiceVersion("2.0");
    invo->setMethodName("sayHello");

    metadata_->setInvocationInfo(invo);
    metadata_->setMessageType(Extensions::NetworkFilters::DubboProxy::MessageType::Request);

    copy->setInvocationInfo(invo);
    copy->setMessageType(Extensions::NetworkFilters::DubboProxy::MessageType::Request);

    return metadata_;
}


Extensions::NetworkFilters::DubboProxy::MessageMetadataSharedPtr AmqpResponseBuilder::message() {

    metadata_->setMessageType(Extensions::NetworkFilters::DubboProxy::MessageType::Response);
    return metadata_;
}

// http callback wrapper templates

template<void (AmqpBuilder::*f)()>
...
//http_parser_settings settingglobal;

//ettingglobal.on_message_begin = &mb_cb<&AmqpBuilder::onMessageBegin>;

/*
        http_parser_settings AmqpBuilder::settings_ = http_parser_settings{
...
http_parser_settings AmqpBuilder::settings_ = http_parser_settings{
        &mb_cb<&AmqpBuilder::onMessageBegin>,
        &mb_data_cb<&AmqpBuilder::onUrl>,
        &mb_data_cb<&AmqpBuilder::onStatus>,
        &mb_data_cb<&AmqpBuilder::onHeaderField>,
        &mb_data_cb<&AmqpBuilder::onHeaderValue>,
        &mb_cb<&AmqpBuilder::onHeadersComplete>,
        &mb_data_cb<&AmqpBuilder::onBody>,
        &mb_cb<&AmqpBuilder::onMessageComplete>,
        nullptr,
        nullptr};

//AmqpBuilder::settings_.on_message_begin = &mb_cb<&AmqpBuilder::onMessageBegin>;


/*
class AmqpBridge : public proton::messaging_handler,
                   public Network::Fiflter,
                   protected Loggable {
public:
...
DubboConverConfig::DubboConverConfig(
...
Network::FilterStatus DubboConverFilter::onData(Buffer::Instance &data,
                                                bool end_stream) {

    ENVOY_LOG(trace, "DubboConverFilter onData {}, cluster info {},Got {}  data {}",
              config_->protocol_,
              local_info_.clusterName(),
              read_callbacks_->connection().nextProtocol(), data.toString());

    dubbo_in_.move(data);
    process(end_stream, false);
    data.move(http_out_); // Replace with HTTP data
    if (dubbo_out_.length()) {
        forceWrite(); // Make sure this data gets written
    }
...
Network::FilterStatus DubboConverFilter::onNewConnection() {
    ENVOY_LOG(trace, "DubboConverFilter onNewConnection {}, Got {}",
              config_->protocol_,
...
Network::FilterStatus DubboConverFilter::onWrite(Buffer::Instance &data, bool end_stream) {

    ENVOY_LOG(trace, "DubboConverFilter onWrite {}, Got {}  data {} end_stream {}",
              config_->protocol_,
              read_callbacks_->connection().nextProtocol(), data.toString(),end_stream);

    ASSERT(http_out_.length() == 0);
    http_in_.move(data);
    ENVOY_LOG(trace, "DubboConverFilter onWrite http_in_ {}",
              http_in_.toString());
    process(false, end_stream);
    ENVOY_LOG(trace, "DubboConverFilter onWrite dubbo_out_ {}",
                      dubbo_out_.toString());
    data.move(dubbo_out_);            // Replace with dubbo data
    ENVOY_LOG(trace, "DubboConverFilter onWrite data {}",
                      data.toString());
    ASSERT(http_out_.length() == 0); // Should not generate HTTP data here.
...
// @param end_amqp: the AMQP-in stream (connection read stream) has ended
// @param end_http: the HTTP-in stream (connection write stream) has ended
void DubboConverFilter::process(bool end_dubbo, bool end_http) {

    ENVOY_LOG(debug, "DubboConverFilter process , end_dubbo {} end_http {}",
            end_dubbo,end_http);

    try {
        processHttp(end_http); // Read http_in_, generate dubbo data
        do {
            processAmqp(); // Read amqp_in_, dispatch events, write amqp_out_
            if (end_http && !http_in_.length()) {
                // Write side locally closed, no more HTTP coming - do polite AMQP
                // protocol close.
                ENVOY_LOG(debug, "No more HTTP data , closing Dubbo connection?");
                //TODO need to close dubbo??
            }
            if (end_dubbo && !dubbo_in_.length()) {
                // Read side remotely closed, inform the driver to run final events.
                ENVOY_LOG(debug, "Read side remotely closed, inform the driver to run final events");
            }
            processHttp(end_http); // AMQP processing may unblock HTTP processing
            // close() calls or processHttp() can generate more events.
        } while (false);
        ...
        // Disaster, slam the Envoy connection shut, throw away everything.
        ENVOY_LOG(error, "unexpected error: {}", e.what());
        conn().close(Network::ConnectionCloseType::NoFlush);
    }
}


void DubboConverFilter::processHttp(bool end_http) {
    if (receiver_ready_) {
        ENVOY_LOG(debug, "DubboConverFilter processHttp ,end_http {}",
                  end_http);
        while (request_.parse(http_in_, end_http)) {

            ENVOY_LOG(debug, "DubboConverFilter processHttp ,after request_ parser {}",
                      end_http);
            //Extensions::NetworkFilters::DubboProxy::MessageMetadataSharedPtr metadata = request_.message();


           // auto invo = std::make_shared<Extensions::NetworkFilters::DubboProxy::RpcInvocationImpl>();
            auto metadata = std::make_shared<Extensions::NetworkFilters::DubboProxy::MessageMetadata>();
 ...
 /*           invo->setServiceName("org.apache.dubbo.samples.http.api.DemoService");
            ...
            metadata->setMessageType(Extensions::NetworkFilters::DubboProxy::MessageType::Request);


            auto cid = next_id_++;
            metadata->setRequestId(cid);
*/
            next_id_++;
/*
            metadata->setResponseStatus(Extensions::NetworkFilters::DubboProxy::ResponseStatus::Ok);
            metadata->setMessageType(Extensions::NetworkFilters::DubboProxy::MessageType::HeartbeatResponse);

            ENVOY_LOG(debug, "DubboConverFilter after request_ parser metadata {} {} {} {} ",
                      metadata->invocationInfo().serviceName(),metadata->invocationInfo().methodName(),
                      metadata->requestId(), metadata->messageType());
            try {
                if (!protocol_->encode(dubbo_out_, *metadata, "")) {
                    ENVOY_LOG(debug, "DubboConverFilter processHttp encode failed to dubbo_out_ ");
                    return;
                }
            } catch (const EnvoyException& e) {
                ENVOY_LOG(debug, "DubboConverFilter protocol_ exction {} ",
                        e.what());
            }
*/

            dubbo_out_.add("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb");

            ENVOY_LOG(debug, "dubbo_out_ length {}", dubbo_out_.length());
...
// or driver is finished.
//
void DubboConverFilter::processAmqp() {
    ENVOY_LOG(debug, "in DubboConverFilter processAmqp");
