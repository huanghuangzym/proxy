...
#include "src/envoy/tcp/dubbotest/config.h"
...
Network::FilterFactoryCb createFilterFactoryHelper(
    const envoy::tcp::dubboconvert::config::DubboConvert& proto_config,
    Server::Configuration::CommonFactoryContext& context,
    FilterDirection filter_direction) {
  //ASSERT(!proto_config.protocol().empty());

    if (proto_config.IsInitialized()) {

    }

    DubboConverConfigSharedPtr filter_config(
      std::make_shared<DubboConverConfig>(
          StatPrefix, "dubbo", filter_direction,
          context.scope()));
  return [filter_config,
          &context](Network::FilterManager& filter_manager) -> void {
    filter_manager.addFilter(std::make_shared<DubboConverFilter>(
...
MetadataExchangeConfigFactory::createFilterFactoryFromProto(
  ...
  return createFilterFactory(
      dynamic_cast<
          const envoy::tcp::dubboconvert::config::DubboConvert&>(
          config),
      context);
}
...
MetadataExchangeConfigFactory::createEmptyConfigProto() {
  return std::make_unique<
      envoy::tcp::dubboconvert::config::DubboConvert>();
}
